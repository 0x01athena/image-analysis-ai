...
### 7. コンディション (condition)
画像を詳細に分析し、商品の状態（傷、汚れ、使用感など）を評価して、以下の6段階から該当する条件を選択してください。
...
**判定ルール：**
- 画像を注意深く観察し、商品の表面、端、縫い目、素材の状態を確認してください。
- 傷、汚れ、変色、ほつれ、破れ、擦れ、変形などの欠陥を探してください。
- 使用感（シワ、型崩れ、毛玉など）も評価に含めてください。
- 複数の画像がある場合は、すべての画像を確認して総合的に判断してください。
- 最も適切な条件レベルを選択し、数字のみ（1、2、3、4、5、6のいずれか）を出力してください。
- 確信が持てない場合でも、画像から判断できる範囲で最も近いと思われる条件を必ず選択してください。

**出力形式：**
数字のみを返してください（例: "1", "2", "3", "4", "5", "6"）。
