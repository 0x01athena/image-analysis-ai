...
B → 画像が不明瞭で正確な特定が難しいが、**部分的にデータが抽出できた** 場合。  
C → 画像の内容に基づかず、**外部情報や推測に依存している** 場合。

出力は必ず "A" / "B" / "C" のいずれか1文字で。
...
**ブランド名の表記ルール：**
- 外国ブランド名は必ずカタカナ表記（例: Chanel → シャネル, Prada → プラダ）。
